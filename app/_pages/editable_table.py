...
def render(table_name: str, username: str) -> None:
    ...
    st.markdown("""
    <style>
    div[data-testid="stHorizontalBlock"] button { border-radius: 6px !important; }
    </style>
    """, unsafe_allow_html=True)

    p_col1, p_col2, p_col3, p_col4, p_col5 = st.columns([1, 1, 3, 1, 1])
    with p_col1:
        if st.button("⏮", disabled=page_num <= 1, key=f"first_{table_name}", use_container_width=True, help="Prima pagina"):
            st.session_state[page_key] = 1
            st.rerun()
    with p_col2:
        if st.button("◀", disabled=page_num <= 1, key=f"prev_{table_name}", use_container_width=True, help="Pagina precedente"):
            st.session_state[page_key] -= 1
            st.rerun()
    with p_col3:
        p_inner1, p_inner2, p_inner3 = st.columns([2, 1, 2])
        with p_inner1:
    ...
    with p_col4:
        if st.button("▶", disabled=page_num >= total_pages, key=f"next_{table_name}", use_container_width=True, help="Pagina successiva"):
            st.session_state[page_key] += 1
            st.rerun()
    with p_col5:
        if st.button("⏭", disabled=page_num >= total_pages, key=f"last_{table_name}", use_container_width=True, help="Ultima pagina"):
